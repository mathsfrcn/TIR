...
\begin{figure}[H]
    ...
    \caption{Example of \ac{BnB} : Mobile Robot Localisation and Mapping in Extensive Outdoor Environments.}%\cite
\end{figure}
...
In a paper titled \underline{A constraint programming approach to the
Zahn’s decision problem}\cite{noguchi_computational_2019} , the \ac{ZP} was transformed into a valued constraint satisfaction problem. This transformation is done as follows : 
\begin{itemize}
...
Discrete integration in high dimensions is considered to be a generally unsolved and difficult problem and has applications to a wide variety of fields. This difficulty arises from the phenomenon known as the curse of dimensionality. In the paper \underline{Taming the Curse of Dimensionality : Discrete Integration by Hashing and Optimization}\cite{ermon_taming_2013}, transform the original problem in set of combinatorial problems solved by Toulbar2.
\newline
...
The \ac{KPCG} is a variant of the knapsack problem where conflict between objects is added. For a given graph $G$, each object is assigned to a vertex and an edge between vertex $i$ and $j$ implies that object $i$ and $j$ cannot be in a solution together. The \ac{KPCG} is difficult to solve with constraint programming as \ac{CFN} do not allow the modeling of linear constraint without a non-trivial performance penalty, especially those with large coefficients.
So and so developed a way to model pseudo boolean linear constraints with \ac{CNF} allowing more problems to be solved using \ac{VSCP}. When toulbar2 was compared with the state-of-the-art \ac{ILP} solver \textit{CPLEX 20.1} for solving the \ac{KPCG}. $6$ classes with $720$ instances each of the problem were tested with different $weights$, $capacities$ and $cost$. The results found that Toulbar2 was more efficient than \textit{CPLEX} in $4$ out $6$ classes. Furthermore, Toulbar2 found the best solutions for the largest number of instances in every class.
...
\begin{figure}[H]
    ...
    \caption{Scaling Neuro-symbolic Problem Solving : Solver-Free Learning of Constraints and Objectives (Defresne et al., 2026)\cite{defresne_scaling_2026}.}
\end{figure}
...
We can also reduce communication overhead when integrating Toulbar2 with SAT solvers for \textit{Satisfiability Modulo Counting} ($SMC^{2}$). Traditional integrated systems run slowly due to continuous communication between the separate logic and probabilistic solvers, this problem is solved with integrated solvers like $\text{KOCO SMC}^{8}$, they improve this by tracking lower and upper bounds during probabilistic inference. They also evaluate partial variable assignments early in order to help to prevent exhaustive enumerations.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/smc_example.png}
    \caption{Example of SMC : Robust Supply Chain with Different Network Size (Li et al., 2025)\cite{li_solving_2025}.}
\end{figure}
